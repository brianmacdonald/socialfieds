...
class ListingManager(models.Manager):
    ...
    def get_latest(self,board=None,category=None,order='-created',user=None):
        """
            ...
            ``user``
                User object. If supplied, method will filter listing from 
                that user.
            
        """
        date_limit = datetime.date.today() - datetime.timedelta(
            days=CLASSIFIEDS_DAY_LIMIT)
        if board and category:
            listings = self.filter(board=board, category=category, 
                is_active=True, user__is_active=True, created__gt=date_limit,
                ).order_by(order)
        elif board is not None and category is None:
            listings = self.filter(board=board, is_active=True, 
                user__is_active=True, created__gt=date_limit,
                ).order_by(order)
        elif user is not None:
            listings = self.filter(is_active=True, created__gt=date_limit,
                user=user, user__is_active=True, ).order_by(order)
        else:
            listings = self.filter(is_active=True, created__gt=date_limit,
...
class Listing(models.Model):
    ...
    def save(self, **kwargs):
        """
        Creates slug and saves listing. 
        """
        if self.slug is None:
            self.slug = slugify(self.name)
            while Listing.objects.filter(slug=self.slug).count() > 0:
