...
def board_categories(request,slug):
    ...
    categories = Category.objects.filter(is_active=True).exclude(
        parent_category=None).order_by('parent_category__title')
...
def inactivate_listing(request, board_slug, category_slug, listing_slug):
    ...
    if request.method == "POST":
        listing = get_object_or_404(Listing, slug=listing_slug, 
            category=category, board=board, user=request.user, is_active=True)
...
def flag_listing(request,board_slug,category_slug,listing_slug):
    ...
    if request.method == "POST" and request.user.username:
        ...
        if form.is_valid():
            flag = ListingFlag(user=request.user,listing=listing, 
